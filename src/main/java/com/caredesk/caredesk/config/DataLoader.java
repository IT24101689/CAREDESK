...
public class DataLoader implements CommandLineRunner {
    ...
    public void run(String... args) throws Exception {
        ...
        if (customerRepository.count() == 0) {
            ...
            customer.setPassword(passwordEncoder.encode("123")); // Encode password
            // Use CustomerService if it has a save method, otherwise direct save
            if (customerService != null && customerService.getClass().getMethod("saveCustomer", Customer.class) != null) {
                customerService.saveCustomer(customer);
            } else {
                customerRepository.save(customer);
            }
            System.out.println("Default customer created: email=john@example.com, password=123 (encoded)");
        }
    }
}